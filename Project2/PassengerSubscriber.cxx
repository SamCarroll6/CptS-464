...
extern "C" int subscriber_main(char *route, int start, int end)
{
    ...
    for (count=0; (0 == 0) || (count < 0); ++count) {
        ...
        //     printf("%d  %d\n", end, hold->numStops);
        // }
        if(ahold && on == 1 && strcmp(vehicle, ahold->vehicle) == 0)
        {
            if(ahold->stopNumber > end)
            {
                remaining = (total + end) - ahold->stopNumber;
            }
            else
            {
                remaining = end - ahold->stopNumber;
            }
            if(strcmp(vehicle, ahold->vehicle) == 0)
            {
                printf("Arriving at stop #%d, at %s, Accident, %d stops remaining\n", ahold->stopNumber, ahold->timestamp, remaining);
            }
            if(strcmp(vehicle, ahold->vehicle) == 0 && ahold->stopNumber == end)
            {
                printf("Accident occured at destination, exiting %s at %s\n", ahold->vehicle, ahold->timestamp);
                break;
            }
            ahold = NULL;
        }
        ...
        {
            if(strcmp(route, hold->route) == 0)
            {
                if(prev != hold->stopNumber)
                {
                    if(hold->stopNumber == start && on == 0)
                    {
                        // remaining = (hold->numStops + end) - hold->stopNumber;
                        if(start > end)
                        {
                            remaining = (hold->numStops + end) - start;
                        }
                        else
                        {
                            remaining = end - hold->stopNumber;
                        }
                        on = 1;
                        strcpy(vehicle, hold->vehicle);
                        printf("Getting on %s at %s, %s, %d stops left\n", hold->vehicle, hold->timestamp, hold->trafficConditions, remaining);
                    }
                    else if(on == 1 && end == hold->stopNumber && strcmp(vehicle, hold->vehicle) == 0)
                    {
                        printf("Arriving at destination by %s at %s\n", hold->vehicle, hold->timestamp);
                        break;
                    }
                    else if(on == 1 && strcmp(vehicle, hold->vehicle) == 0)
                    {
                        if(hold->stopNumber > end)
                        {
                            remaining = (hold->numStops + end) - hold->stopNumber;
                        }
                        else
                        ...
                        printf("Arriving at stop #%d, at %s, %s, %d stops remaining\n", hold->stopNumber, hold->timestamp, hold->trafficConditions, remaining);
                    }
                    //prev = hold->stopNumber;
                }
                total = hold->numStops;
            }
            hold = NULL;
